...
import { Trash2, Plus, ImageIcon, ArrowUp, ArrowDown, FileText, Loader2 } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
  ...
  const importM3uPlaylist = async () => {
    ...
    try {
      ...
      if (!data.streams || !Array.isArray(data.streams) || data.streams.length === 0) {
        throw new Error("No valid streams found in the playlist")
      }

      // Add the imported streams to the existing streams
      const startPriority = streams.length + 1
      const importedStreams = data.streams.map((stream: any, index: number) => ({
        id: Math.random().toString(36).substring(2, 9),
        url: stream.url,
